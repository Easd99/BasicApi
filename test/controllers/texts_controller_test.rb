...
class TextsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create text" do
    assert_difference('Text.count') do
      post texts_url, params: { text: { description: @text.description, title: @text.title } }, as: :json
    end
  ...
  test "should update text" do
    patch text_url(@text), params: { text: { description: @text.description, title: @text.title } }, as: :json
    assert_response 200
  end
